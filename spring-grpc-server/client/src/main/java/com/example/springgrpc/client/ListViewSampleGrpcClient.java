 ...
 * 여기서는 {@link ListViewRequest}를 그대로 받는다. 필터가 재귀 트리라 원시 타입으로 감싸면
...
public final class ListViewSampleGrpcClient extends AbstractGrpcClient {
    ...
    public ListViewResponse listAll(ListViewRequest request) {
        return listAll(request, Map.of());
    }

    public ListViewResponse listAll(ListViewRequest request, Map<String, String> headers) {
        if (request == null) {
