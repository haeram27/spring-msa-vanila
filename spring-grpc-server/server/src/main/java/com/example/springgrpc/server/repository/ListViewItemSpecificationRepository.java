...
import com.example.springgrpc.server.repository.specification.ListViewSortSpecifications;
import com.example.springgrpc.server.repository.specification.ListViewSpecifications;
 ...
 * {@link ListViewItemQueryRepository}(QueryDSL)와 같은 입출력 계약을 갖는 대체 구현이다.
 * <p>
 * 필터와 정렬을 각각 {@link ListViewSpecifications}, {@link ListViewSortSpecifications}로 만들어
 ...
 * 서비스는 {@link ListViewQuery}를 주고 {@link ListViewResult}를 받을 뿐이다.
 */
@Repository
public class ListViewItemSpecificationRepository {

    private final ListViewItemRepository listViewItemRepository;

    public ListViewItemSpecificationRepository(ListViewItemRepository listViewItemRepository) {
     ...
     * @param pageSize   1 이상
     */
    public ListViewResult search(ListViewQuery query, int pageNumber, int pageSize) {
        ListViewQuery.Sort sort = query.sort();
        Specification<ListViewItemEntity> specification = ListViewSpecifications.fromFilter(query.filter())
            .and(ListViewSortSpecifications.orderBy(toSortKey(sort), toSortOrder(sort)));

        // PageRequest 는 0부터 시작하는 페이지 번호를 쓴다. 정렬은 Specification 안에서 이미
        // 설정하므로 Pageable 에는 Sort 를 싣지 않는다.
        Page<ListViewItemEntity> page =
            listViewItemRepository.findAll(specification, PageRequest.of(pageNumber - 1, pageSize));

        return new ListViewResult(
            page.getContent().stream().map(ListViewItemEntity::toDomain).toList(),
            (int) page.getTotalElements());
    }

    /**
     * 도메인 정렬 키를 {@link ListViewSortSpecifications}의 화이트리스트 값으로 옮긴다.
     * <p>
     * {@code name().toLowerCase()} 대신 명시적 switch를 쓴다. 도메인에 키가 추가되면 런타임에
     * "Unknown sort_key value"로 조용히 실패하는 대신 컴파일 에러로 드러난다.
     */
    private static String toSortKey(ListViewQuery.Sort sort) {
    ...
    private static String toSortOrder(ListViewQuery.Sort sort) {
