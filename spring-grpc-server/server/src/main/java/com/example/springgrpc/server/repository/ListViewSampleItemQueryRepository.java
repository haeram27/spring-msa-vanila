 ...
 * <p>
 * QueryDSL과 JPA 엔티티는 이 패키지 밖으로 나가지 않는다. 서비스는 {@link ListViewSampleQuery}를 주고
 * {@link ListViewSampleResult}를 받을 뿐이라, 저장소를 바꿔도 서비스와 어댑터는 그대로다.
 * <p>
 * 필터가 재귀 트리라서 문자열 JPQL로는 조립이 지저분해진다. QueryDSL의
 * {@link BooleanBuilder}는 트리 구조를 그대로 따라가며 조건을 합칠 수 있어 이 모양에 잘 맞는다.
 */
@Repository
public class ListViewSampleItemQueryRepository {

    private static final QListViewSampleItemEntity ITEM = QListViewSampleItemEntity.listViewSampleItemEntity;

    private final JPAQueryFactory queryFactory;

    public ListViewSampleItemQueryRepository(JPAQueryFactory queryFactory) {
     ...
     * @param offset   0 이상
     */
    public ListViewSampleResult search(ListViewSampleQuery query, int offset, int pageSize) {
        BooleanBuilder where = new BooleanBuilder(toPredicate(query.filter()));

        List<ListViewSampleEntry> items = queryFactory
            .selectFrom(ITEM)
            .where(where)
        ...
        return new ListViewSampleResult(items, total == null ? 0 : total.intValue());
    }
     ...
     */
    private static Predicate toPredicate(ListViewSampleFilter filter) {
        if (filter == null) {
            return null;
        }

        return switch (filter) {
            case ListViewSampleFilter.And and -> {
     ...
     * 항상 id를 뒤에 덧붙인다.
     */
    private static OrderSpecifier<?>[] toOrder(ListViewSampleQuery.Sort sort) {
        boolean asc = sort == null || sort.order() != ListViewSampleQuery.Sort.Order.DESC;
        ListViewSampleQuery.Sort.Key key = sort == null ? ListViewSampleQuery.Sort.Key.ID : sort.key();

        OrderSpecifier<?> tiebreaker = asc ? ITEM.id.asc() : ITEM.id.desc();
        if (key == ListViewSampleQuery.Sort.Key.ID) {
