...
@Component
class SpringLifeCycleEventListener(
    ...
    fun onEventListener(event: ApplicationEvent) {
        log.trace { "${applicationContext.applicationName}: onEventListener: ${event.javaClass.simpleName}" }
    ...
    override fun onApplicationEvent(event: ApplicationEvent) {
        log.trace { "${applicationContext.applicationName}: onApplicationEvent: ${event.javaClass.simpleName}" }
        if (event is ApplicationReadyEvent) {
            return
        }
        if (event is ContextClosedEvent) {
