...
@SpringBootTest
class ApacheRestTemplateSyncTests : EvaluatedTimeTests() {
    ...
    companion object {
        ...
        private const val API_URL_PATH = "/todos"
    ...
    fun sendResponseMapToJsonNode(method: HttpMethod, uri: URI, httpEntity: HttpEntity<Void>): JsonNode {
        ...
        try {
            ...
            log.debug {
                "Response http code: ${responseEntity.statusCode.value()}, reason: " +
                    "${HttpStatus.valueOf(responseEntity.statusCode.value()).reasonPhrase}"
            }
            ...
            if (status.is4xxClientError) {
                log.error { "Error: ${ex.responseBodyAsString}" }
            } else {
                log.error { "Error: ${ex.responseBodyAsString}" }
            }
